...
const isLocalhost = Boolean(
  ...
  export function register(config) {
    if (process.env.NODE_ENV === 'production' && 'serviceWorker' in navigator) {
      // Ensure the public URL is correctly set for subdirectory deployment
      ...
      if (publicUrl && new URL(publicUrl, window.location.href).origin !== window.location.origin) {
        // The service worker won't work if PUBLIC_URL is on a different origin from what our page is served on.
        return;
      }
      ...
      window.addEventListener('load', () => {
        if (isLocalhost) {
          ...
          // Log additional information when running on localhost.
